...
#
# Multi-stage build for the cf-rs binary, per
...
#                actual cf-rs binary for the musl target matching the
...
# are not needed to build the cf-rs binary; .dockerignore excludes it from
...
# cf-rs container to launch function containers/builds, e.g.:
#   docker run \
...
# container as on the host, since cf-rs resolves function source paths as
...
# Now build the actual cf-rs binary against the already-cooked dependencies,
# then copy it to a target-triple-independent path so the runtime stage
# below doesn't need its own TARGETARCH branching.
COPY . .
RUN cargo zigbuild --release --target "$(cat /tmp/rust_target)" -p cf-rs \
    && cp "target/$(cat /tmp/rust_target)/release/cf-rs" /app/cf-rs
...
COPY --from=builder /app/cf-rs /usr/local/bin/cf-rs
COPY docker/config.toml /etc/cf-rs/config.toml

USER nonroot
ENTRYPOINT ["/usr/local/bin/cf-rs"]
CMD ["serve"]
EXPOSE 8080 8081
VOLUME /data
HEALTHCHECK CMD ["/usr/local/bin/cf-rs", "health"]
STOPSIGNAL SIGTERM
