...
`cf-rs-sdk` is a Rust implementation of the [Google Cloud Functions
...
- **Locally**, hosted by [`cf-rs`](https://crates.io/crates/cf-rs) (this
  crate's sibling in the `cf-rs` workspace), which builds/runs the function
...
- **On Google Cloud Run functions**, deployed as-is via `gcloud run deploy`
  ...
  compilation, no cf-rs-specific glue.
...
[`Functions`]: https://docs.rs/cf-rs-sdk/latest/cf_rs_sdk/struct.Functions.html
[`axum`]: https://docs.rs/axum
[`CloudEvent`]: https://docs.rs/cf-rs-sdk/latest/cf_rs_sdk/type.CloudEvent.html
...
[`MessagePublishedData`]: https://docs.rs/cf-rs-sdk/latest/cf_rs_sdk/pubsub/struct.MessagePublishedData.html
[`PubsubMessage`]: https://docs.rs/cf-rs-sdk/latest/cf_rs_sdk/pubsub/struct.PubsubMessage.html
[`CloudEventExt::data_as`]: https://docs.rs/cf-rs-sdk/latest/cf_rs_sdk/cloudevent/trait.CloudEventExt.html#tymethod.data_as

## Running locally

### Standalone, without cf-rs
...
useful for testing the SDK layer in isolation, without cf-rs or a real
...
`ce-source`, `ce-type`, ...) reaches the handler the same way cf-rs or
Eventarc would deliver it.

### Via cf-rs

The normal path is to let `cf-rs` build and run the function — it resolves
...
for the full `cf-rs` CLI walkthrough, including Pub/Sub triggers, container
...
exactly mirroring what `cf-rs` sets when running the function as a
...
input to both `cf-rs fn deploy` and `gcloud run deploy`.
...
- `execution_id` is populated automatically for any log line emitted while
  ...
  `Function-Execution-Id` header (set by cf-rs, and by Cloud Run's own
  ...
  handler picks it up.
- `cf-rs fn logs <name>` (or Cloud Logging, on Cloud Run) parses these JSON
...
[`logging::init()`]: https://docs.rs/cf-rs-sdk/latest/cf_rs_sdk/logging/fn.init.html
[`logging::execution_id_middleware`]: https://docs.rs/cf-rs-sdk/latest/cf_rs_sdk/logging/fn.execution_id_middleware.html

## Error handling

`Functions::serve()` returns `Result<(), cf_rs_sdk::Error>`; a `main`
...
Framework's own startup-failure behavior, which both `cf-rs` and Cloud Run
