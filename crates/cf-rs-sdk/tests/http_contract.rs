//! Contract tests for `cf-rs-sdk`'s HTTP routing (T011), per
...
//! `expect_used` lints are relaxed here, matching `crates/cf-rs/tests/config.rs`.
...
use cf_rs_sdk::{Error, Functions, HttpRequest, HttpResponse};
...
/// (which installs a *process global* subscriber cf-rs-sdk itself must own) or adding a
