...
pub struct InstancePool {
    ...
    /// Label value for `open_functions_cold_start_seconds{driver}` (T082/US5):
...
impl InstancePool {
    ...
    pub async fn acquire(&self) -> Result<AcquiredInstance, AcquireError> {
        ...
        loop {
            if let Some(acquired) = self.try_acquire_existing().await {
                metrics::histogram!("open_functions_queue_wait_seconds", "function" => self.function_name.clone())
    ...
    /// Refreshes `open_functions_instances{function,state="ready"}` (T082/US5) after a
