...
/// The Docker network open-functions creates (if missing) and connects every
/// image-mode function container to, per plan.md: create the `open-functions` network
/// at startup if missing. If open-functions itself runs inside Docker, Ansible connects the
/// open-functions container to this same network so the reverse proxy can reach
/// function containers by IP.
pub const NETWORK_NAME: &str = "open-functions";

/// Label applied to every container open-functions creates, keyed by function name.
...
pub const LABEL_FUNCTION: &str = "open-functions.function";
