...
//! "Status codes" table. It deliberately has no `axum` dependency (`open-functions-core`
...
//! `hyper-util`) is implemented by the `Forwarder` in the `open-functions` binary crate,
...
pub struct RequestRewriteContext {
    ...
    /// "http" or "https", forwarded as `X-Forwarded-Proto`. open-functions itself has
