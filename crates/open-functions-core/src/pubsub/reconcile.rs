...
fn subscription_name(function_name: &str) -> String {
    format!("open-functions-{function_name}")
}
...
impl Reconciler {
    ...
    /// Recomputes and publishes the `open_functions_pubsub_bindings` gauge (one time
    ...
    fn report_binding_gauge(&self) {
        ...
        metrics::gauge!("open_functions_pubsub_bindings", "state" => "pending")
            .set(counts[0] as f64);
        metrics::gauge!("open_functions_pubsub_bindings", "state" => "bound").set(counts[1] as f64);
        metrics::gauge!("open_functions_pubsub_bindings", "state" => "unbinding")
            .set(counts[2] as f64);
        metrics::gauge!("open_functions_pubsub_bindings", "state" => "error").set(counts[3] as f64);
    }
