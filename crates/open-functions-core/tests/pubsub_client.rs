...
fn sample_subscription_json(push_endpoint: &str) -> Value {
    json!({
        "name": "projects/test-project/subscriptions/open-functions-my-fn",
...
async fn create_subscription_2xx_returns_ok() {
    ...
    Mock::given(method("PUT"))
        .and(path(format!(
            "/v1/projects/{PROJECT}/subscriptions/open-functions-my-fn"
        )))
        .and(body_json(json!({
            "topic": req.topic,
    ...
    let sub = ok(
        client
            .create_subscription("open-functions-my-fn", &req)
            .await,
        "create_subscription",
    );
    ...
    assert_eq!(
        sub.name,
        "projects/test-project/subscriptions/open-functions-my-fn"
    );
...
async fn create_subscription_unreachable_returns_unreachable_error() {
    ...
    let e = err(
        client
            .create_subscription("open-functions-my-fn", &sample_request())
            .await,
...
async fn get_subscription_200_returns_some_with_fields_populated() {
    ...
    let sub = some(
        ok(
            client.get_subscription("open-functions-my-fn").await,
            "get_subscription",
        ),
...
async fn get_subscription_404_returns_none() {
    ...
    Mock::given(method("GET"))
        .and(path(format!(
            "/v1/projects/{PROJECT}/subscriptions/open-functions-missing"
        )))
    ...
    let client = client_for(server.uri());
    let sub = ok(
        client.get_subscription("open-functions-missing").await,
...
async fn delete_subscription_204_returns_ok() {
    ...
    let client = client_for(server.uri());
    ok(
        client.delete_subscription("open-functions-my-fn").await,
...
async fn delete_subscription_404_is_treated_as_success() {
    ...
    let client = client_for(server.uri());
    ok(
        client.delete_subscription("open-functions-missing").await,
...
async fn recreate_subscription_deletes_then_puts() {
    ...
    let sub = ok(
        client
            .recreate_subscription("open-functions-my-fn", &req)
            .await,
