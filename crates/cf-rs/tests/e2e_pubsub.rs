...
//! stdout log line, captured through cf-rs's own process — see
//! `runtime::process::spawn_log_drain`, which re-emits a function instance's
//! captured stdout as a `tracing` event on `cf-rs serve`'s own stdout), then
...
//! Set `CF_RS_TEST_PSRS_URL` (e.g. `http://127.0.0.1:8085`) to run it.
//!
...
//! (`crates/cf-rs-core/src/pubsub/client.rs`); adjust here if ps-rs's actual
//! topics API differs.
...
/// A running `cf-rs serve` subprocess with its stdout captured line-by-line
...
/// `cf-rs serve` re-emits as its own `tracing` events.
struct ServeProcess {
    child: Child,
    admin_url: String,
...
async fn spawn_serve(
    ...
    let mut child = Command::new(bin)
        ...
        .env("CF_RS__PUBSUB__ENABLED", "true")
        .env("CF_RS__PUBSUB__BASE_URL", psrs_url)
        .env("CF_RS__PUBSUB__PROJECT", project)
        .env("CF_RS__PUBSUB__RETRY_INITIAL_SECS", "1")
        .env("CF_RS__PUBSUB__RETRY_MAX_SECS", "5")
        .stdout(Stdio::piped())
        .stderr(Stdio::null())
        .spawn()
...
async fn topic_publish_reaches_function_and_delete_removes_subscription() {
    let Ok(psrs_url) = std::env::var("CF_RS_TEST_PSRS_URL") else {
        eprintln!(
            "skipping topic_publish_reaches_function_and_delete_removes_subscription: \
             set CF_RS_TEST_PSRS_URL (e.g. http://127.0.0.1:8085) to run against a real ps-rs"
        );
        return;
    };
    let psrs_url = psrs_url.trim_end_matches('/').to_string();
    let project = "local";
    let topic = format!("cf-rs-e2e-{}", uuid::Uuid::new_v4().simple());
