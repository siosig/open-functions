//! Integration tests for `cf-rs`'s configuration loader (`src/config.rs`).
//!
//! `cf-rs` is a binary-only crate (no `lib` target), so this test includes the
//! source module directly via `#[path]` rather than importing it as `cf_rs::config`.
//!
...
/// calls `config::load`, whose behavior depends on `CF_RS_CONFIG` and any
/// `CF_RS__*` variable — including tests that only assert on *defaults*, which
/// would observe another thread's `CF_RS__ADMIN__LISTEN` override mid-flight
...
/// Ensures `CF_RS_CONFIG` does not leak into a test run. Callers must hold
/// `ENV_MUTEX` before calling this (and before touching any `CF_RS__*` var).
fn clear_process_env() {
    // SAFETY: caller holds ENV_MUTEX, so no other test's set/remove of any
    // CF_RS*-prefixed variable can run concurrently with this one.
    unsafe {
        std::env::remove_var("CF_RS_CONFIG");
    }
}
...
fn defaults_only_load_produces_documented_defaults() {
    ...
    assert_eq!(cfg.storage.data_dir, "/var/lib/cf-rs");
...
fn env_var_override_wins_over_default() {
    ...
    unsafe {
        std::env::set_var("CF_RS__ADMIN__LISTEN", "127.0.0.1:9999");
    }
    ...
    unsafe {
        std::env::remove_var("CF_RS__ADMIN__LISTEN");
    }
