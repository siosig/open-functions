//! Contract tests for `open-functions-sdk`'s CloudEvent routing (T047), per
...
//! `expect_used` lints are relaxed here, matching `crates/open-functions-sdk/tests/http_contract.rs`.
...
use open_functions_sdk::{CloudEvent, CloudEventExt, Error, Functions};
...
/// `open-functions-core`'s host-side Pub/Sub → CloudEvent conversion emits (see
/// `crates/open-functions-core/src/pubsub/convert.rs`) and real Cloud Pub/Sub sends.
...
fn assert_decodes_pubsub_data(ev: &CloudEvent) {
    ...
    assert_eq!(
        data.subscription,
        "projects/local/subscriptions/open-functions-on-msg"
    );
}
