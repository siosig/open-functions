...
//! set `CF_RS_TEST_DOCKER=1` to run, matching this workspace's existing
//! opt-in-external-dependency convention (see `crates/cf-rs/tests/e2e_pubsub.rs`'s
//! `CF_RS_TEST_PSRS_URL` gate). `.github/workflows/ci.yml`'s `docker-tests`
...
use cf_rs_core::runtime::container::{ContainerDriver, sweep_stale_containers};
use cf_rs_core::runtime::docker::{LABEL_FUNCTION, NETWORK_NAME, connect};
use cf_rs_core::runtime::{Driver, InstanceExit, InstanceSpec};

/// The throwaway image tag this test suite builds and reuses across tests.
const TEST_IMAGE_TAG: &str = "cf-rs-test-hello-http:latest";

/// Skips the calling test unless `CF_RS_TEST_DOCKER=1`, per this workspace's
/// opt-in-external-dependency convention.
macro_rules! require_docker_test {
    () => {
        if std::env::var("CF_RS_TEST_DOCKER").is_err() {
            eprintln!(
                "skipping {}: set CF_RS_TEST_DOCKER=1 to run (needs a real Docker daemon)",
                module_path!()
            );
            return;
        }
...
/// `cf-rs` network creates it, the resulting container is reachable over
...
async fn spawn_creates_network_and_serves_http_then_stop_removes_container() {
    ...
    // Best-effort: remove the `cf-rs` network first (if it exists and has no
    ...
    let container_name_prefix = "cf-rs-hello-1-";
...
async fn sweep_stale_containers_removes_labeled_leftovers() {
    ...
    let stale_name = "cf-rs-test-stale-leftover";
