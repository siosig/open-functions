...
//! minutes. Set `CF_RS_TEST_DOCKER` (to any value) to run it, mirroring the
//! `CF_RS_TEST_PSRS_URL` opt-in idiom used by `crates/cf-rs/tests/e2e_pubsub.rs`
...
//! real `examples/hello-http/Cargo.toml` depends on `cf-rs-sdk` via the
//! relative path `../../crates/cf-rs-sdk`, which escapes `examples/hello-http`
...
//! of `crates/cf-rs-sdk` into one fixture directory, with the copied
...
use cf_rs_core::build::{BuildRequest, Builder};
...
fn workspace_root() -> PathBuf {
    ...
    // CARGO_MANIFEST_DIR is `<workspace_root>/crates/cf-rs-core`.
...
/// path dependencies (like `cf-rs-sdk` here) don't embed the dependency's
/// on-disk path, only its name/version/dependency-edges, so the copied
/// lockfile stays valid even once `cf-rs-sdk` is vendored to a new path
/// below.
...
/// A standalone `cf-rs-sdk` manifest with every `.workspace = true`
...
const STANDALONE_CF_RS_SDK_MANIFEST: &str = r#"[package]
name = "cf-rs-sdk"
version = "0.1.0"
edition = "2024"
...
fn build_self_contained_fixture(dest: &Path) -> PathBuf {
    ...
    let vendored_sdk_dir = dest.join("vendor/cf-rs-sdk");
    copy_source_tree(&root.join("crates/cf-rs-sdk"), &vendored_sdk_dir);
    ok(
        std::fs::write(
            vendored_sdk_dir.join("Cargo.toml"),
            STANDALONE_CF_RS_SDK_MANIFEST,
        ),
        "write standalone cf-rs-sdk Cargo.toml",
    );
    ...
    let rewritten = manifest.replace(
        "cf-rs-sdk = { path = \"../../crates/cf-rs-sdk\" }",
        "cf-rs-sdk = { path = \"vendor/cf-rs-sdk\" }",
    );
    assert_ne!(
        manifest, rewritten,
        "expected to find and rewrite the cf-rs-sdk path dependency in {cargo_toml_path:?}"
    );
    ok(
...
fn docker_socket() -> String {
    std::env::var("CF_RS_TEST_DOCKER_SOCKET").unwrap_or_default()
}

#[tokio::test]
async fn successful_build_produces_executable_artifact_and_reuses_the_registry_cache() {
    if std::env::var("CF_RS_TEST_DOCKER").is_err() {
        eprintln!(
            "skipping: set CF_RS_TEST_DOCKER (e.g. CF_RS_TEST_DOCKER=1) to run against a real Docker daemon"
        );
        return;
    }
