...
use cf_rs_core::build::{BuildError, BuildRequest, Builder};
...
/// `CARGO_MANIFEST_DIR` at test-run time is `<workspace_root>/crates/cf-rs-core`;
...
/// on `cf-rs-sdk` via the relative path `../../crates/cf-rs-sdk`; any copy of
...
/// cost of `cf-rs-sdk`'s dependency tree; every later build is incremental.
...
async fn build_with_compile_error_fails_and_logs_the_error() {
    ...
    // system tempdir: its Cargo.toml depends on `cf-rs-sdk` via the relative
    // path `../../crates/cf-rs-sdk`, which only resolves at that depth.
    let copy_holder = ok(
