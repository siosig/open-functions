...
/// Path prefix for cf-rs-managed cgroups under the v2 hierarchy root, per
/// plan.md's Design Notes: `cf-rs/<function_name>/<instance_id>`.
const ROOT: &str = "cf-rs";
...
impl CgroupLimiter {
    ...
    /// `cf-rs/<function_name>/<instance_id>`, sets `memory.max`, and adds
    /// `pid` to it.
    ///
...
mod tests {
    ...
    /// (confirmed manually: cgroup v2 is mounted but `cf-rs/...` cannot be
