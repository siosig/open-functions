        ...
        tokio::spawn(async move {
            ...
            let exit = tokio::select! {
                ...
                Ok(grace) = &mut stop_rx => {
                    ...
                    {
                        tracing::warn!(
                            target: "cf_rs::runtime_container",
...
fn container_env(spec: &InstanceSpec) -> Vec<String> {
    ...
    env.push(format!("CF_RS_MEMORY_MIB={}", spec.memory_mib));
    env
}
...
/// per plan.md: `inspect` reads `NetworkSettings.Networks["cf-rs"].IPAddress`.
...
/// `cf-rs serve` startup (T075/T076, not this module's concern) to clean up
