...
/// Requires a real, reachable Docker daemon -- gated on `CF_RS_TEST_DOCKER`,
...
async fn successful_image_registration_records_the_digest() {
    let Ok(_) = std::env::var("CF_RS_TEST_DOCKER") else {
        eprintln!(
            "skipping successful_image_registration_records_the_digest: \
             set CF_RS_TEST_DOCKER=1 to run against the real local Docker daemon"
        );
        return;
    };
    ...
    // actually behave like a cf-rs function (no instance is started here,
