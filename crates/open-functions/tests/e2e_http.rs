//! End-to-end test for User Story 1 (T031): spawns the real `open-functions` binary,
...
//! over) and serial by nature (one `open-functions serve` process, fixed ports chosen
...
//! `open-functions-sdk`'s full dependency tree — axum, cloudevents-sdk, tokio, ...)
...
async fn failed_redeploy_keeps_previous_revision_serving() {
    ...
    // under examples/ so its relative `path = "../../crates/open-functions-sdk"`
