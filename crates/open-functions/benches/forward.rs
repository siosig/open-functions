...
//! `open_functions_forward_overhead_seconds` per `open_functions_core::forward`'s doc comments
...
//!
//! Run with `cargo bench --bench forward --features bench -p open-functions`
...
use open_functions_core::forward::RequestRewriteContext;
use tokio::net::TcpListener;
use tokio::runtime::Runtime;

// `open-functions` has no lib target (binary crate only, see src/main.rs), so pull
