//! Configuration schema and loader for `open-functions`.
//!
//! Layering order (per `specs/001-cloud-functions-local/contracts/ops-config.md`):
//! built-in defaults < config file < `OPEN_FUNCTIONS__`-prefixed environment variables.
//!
...
impl StorageConfig {
    fn default_data_dir() -> String {
        "/var/lib/open-functions".to_string()
    }
}
...
/// 3. `OPEN_FUNCTIONS__`-prefixed environment variables (double underscore as section separator)
///
...
/// - else `OPEN_FUNCTIONS_CONFIG` env var, if set, is used and must exist/parse.
/// - else `/etc/open-functions/config.toml` is used **only if it exists**; if absent, no file is loaded
...
pub fn load(config_path: Option<&Path>) -> Result<AppConfig, ConfigError> {
    let (path, required) = match config_path {
        ...
        None => match std::env::var("OPEN_FUNCTIONS_CONFIG") {
            Ok(v) if !v.is_empty() => (Some(std::path::PathBuf::from(v)), true),
            _ => {
                let default_path = std::path::PathBuf::from("/etc/open-functions/config.toml");
    ...
    builder =
        builder.add_source(config::Environment::with_prefix("OPEN_FUNCTIONS").separator("__"));
