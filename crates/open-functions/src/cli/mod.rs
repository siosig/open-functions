...
fn run_fn(command: FnCommand) -> ExitCode {
    ...
    // documented in admin-api.md as environment-first (`OPEN_FUNCTIONS_ADMIN_URL`,
    // `OPEN_FUNCTIONS_ADMIN_TOKEN`) with `--admin-url`/`--admin-token` overrides.
    let admin_url = std::env::var("OPEN_FUNCTIONS_ADMIN_URL")
        .unwrap_or_else(|_| "http://127.0.0.1:8081".to_string());
    let admin_token = std::env::var("OPEN_FUNCTIONS_ADMIN_TOKEN").ok();
...
fn print_version() {
    ...
    println!("open-functions {}", env!("CARGO_PKG_VERSION"));
}
